...
# Reference, thank you !
# https://github.com/w86763777/pytorch-gan-metrics/blob/master/pytorch_gan_metrics/inception.py
...
def get_fid(real_imgs, generated_imgs):
    print(real_imgs.shape, generated_imgs.shape)
    real_mu, real_sigma = np.mean(real_imgs, axis=0), np.cov(real_imgs, rowvar=False)
    real_mu = np.atleast_1d(real_mu)
    real_sigma = np.atleast_2d(real_sigma)
    generated_mu, generated_sigma = np.mean(generated_imgs, axis=0), np.cov(generated_imgs, rowvar=False)
    generated_mu = np.atleast_1d(generated_mu)
    generated_sigma = np.atleast_2d(generated_sigma)

    ssdiff = np.sum((real_mu - generated_mu) ** 2.)
    covmean = linalg.sqrtm(real_sigma.dot(generated_sigma))
    
    if np.iscomplexobj(covmean):
        covmean = covmean.real
    fid = ssdiff + np.trace(real_sigma + generated_sigma - 2. * covmean)
    return fid
